...
<body>
    ...
    <div class="row">
        ...
        <div class="table-responsive" >
            ...
            <div class="box box-primary" src="js/loading.gif">
                <!-- Tabs within a box -->

                <ul class="nav nav-tabs pull-left">
                    <li class="pull-left header">
                        <class="fa"> <a href='#' onclick="deleteGraph('<?php echo $graph_query->getId(); ?>');"
                                title="Tutup">
                                <img src="img/icon-delete.gif"></a>&nbsp; &nbsp;<img
                                src="img/icon/icon-chart.png"></i>&nbsp;
                            <strong>
                                <?php
                    ...
                    <ul class="nav nav-tabs pull-right">
                        <li class="pull-right header">
                            ...
                            if ($graph_query->getMonth() == 1) {
                                ?>
                                <select name="month<?php echo $graph_query->getId(); ?>"
                                    id="month<?php echo $graph_query->getId(); ?>">
                                ...
                                ?>
                                <input type='hidden' name="month<?php echo $graph_query->getId(); ?>"
                                    id="month<?php echo $graph_query->getId(); ?>" value='<?php echo date('m') ?>'>
                                <?php
                            }
                            if ($graph_query->getYear() == 1) {
                                ?>
                                <input type="text" name="year<?php echo $graph_query->getId(); ?>"
                                    id="year<?php echo $graph_query->getId(); ?>" value="<?php echo date('Y') ?>" size="5">
                                <?php
                            }
                            ?>
                            <input type="button" value=" Show "
                                onclick="showGraph('<?php echo $graph_query->getId(); ?>','month<?php echo $graph_query->getId(); ?>', 'year<?php echo $graph_query->getId(); ?>')">&nbsp;
                        </li>
                    </ul>
                    <?php
                }
