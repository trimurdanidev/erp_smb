...
<body>
    <?php 
        $mdl_graph_query = new graph_query();
        $ctrl_graph_query = new graph_queryController($mdl_graph_query,$this->dbh);
        $graph_list = $ctrl_graph_query->showDataAll();
    ?>
    <div class="table-responsive">
        <form name="frmgraph_query" id="frmgraph_query" method="post"
            action="index.php?model=dashboard_user&action=saveGraphUserById">
            <table width="100%" border="0" align="right" style="border-collapse: collapse">
                <tr>
                    <td align="right">Personalize Dashboard :
                        ...
                        <input type="hidden" name="user" id="user" value="<?php echo $master_user->getId(); ?>"
                            size="2">
