...
<table width="95%" >
    <tr>
        <td align="left">
                    <img alt="Move First"  src="./img/icon/icon_move_first.gif" onclick="showMenu('content', 'index.php?model=master_product&action=showAllJQuery&search=<?php echo $search ?>');" >
                    <img alt="Move Previous" src="./img/icon/icon_move_prev.gif" onclick="showMenu('content', 'index.php?model=master_product&action=showAllJQuery&skip=<?php echo $previous ?>&search=<?php echo $search ?>');">
                     Page <?php echo $pageactive ?> / <?php echo $pagecount ?> 
                    <img alt="Move Next" src="./img/icon/icon_move_next.gif" onclick="showMenu('content', 'index.php?model=master_product&action=showAllJQuery&skip=<?php echo $next ?>&search=<?php echo $search ?>');" >
                    <img alt="Move Last" src="./img/icon/icon_move_last.gif" onclick="showMenu('content', 'index.php?model=master_product&action=showAllJQuery&skip=<?php echo $last ?>&search=<?php echo $search ?>');">
                    <a href="index.php?model=master_product&action=export&search=<?php echo $search ?>">Export</a>
                    <a href="index.php?model=master_product&action=printdata&search=<?php echo $search ?>" target="_"><img src="./images/icon_print.png"/></a>
        </td>
        <td align="right">
...
<?php if($isadmin || $ispublic || $isentry){ ?>
       <input type="button" class="btn btn-warning btn-sm" value="new" name="new" onclick="showMenu('header_list', 'index.php?model=master_product&action=showFormJQuery')"> 
<?php } ?>
...
<td>&nbsp;</td>
    ...
    ?>
            <tr bgcolor="<?php echo $bg;?>">
                <td><a href='#' onclick="showMenu('header_list', 'index.php?model=master_product&action=showDetailJQuery&id=<?php echo $master_product->getId();?>')"><?php echo $master_product->getId();?></a> </td>
            ...
            <td><a href="" data-toggle="modal"
                ...
                <?php }?></a>
            <div class="modal fade" id="modal_gambar<?php echo $master_product->getId(); ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
            <div class="modal-dialog" style="width:1000px">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel"><?php echo $master_product->getNm_product();?></h5>
                    ...
                    <div class="modal-body" align="center">
                        <?php if($master_product->getImage_product()!=""){?>
                        <img src="<?php echo $master_product->getImage_product();?>" style="align:center"/>
                        <?php }else{?>
  ...
  <?php if($isadmin || $ispublic || $isupdate){ ?>
                    <a href='#' class="btn btn-sm btn-info" data-toggle="modal" data-target="#modal_edit<?php echo $master_product->getId(); ?>" >Edit</a> | 
                    <div class="modal fade" id="modal_edit<?php echo $master_product->getId(); ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog" style="width:1000px">
                            <div class="modal-content">
                                <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" value="<?php echo $master_product->getKd_product(); ?>">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlInput1">Nama Produk</label>
                                <input type="text" class="form-control" value="<?php echo $master_product->getNm_product(); ?>">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlTextarea1">Harga Modal</label>
                                <input type="text" class="form-control" value="<?php echo number_format($master_product->getHrg_modal()); ?>">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlInput1">Harga Jual</label>
                                <input type="text" class="form-control" value="<?php echo number_format($master_product->getHrg_jual()); ?>">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlInput1">Kategori Produk</label>
                                <input type="text" class="form-control" value="<?php echo $showKategoriProduk->getKategori_name(); ?>">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlInput1">Tipe Produk</label>
                                <input type="text" class="form-control" value="<?php echo $showTipeProduk->getTipe_name(); ?>">
                            </div>
                        </form>
                    </div>
...
<?php if($isadmin || $ispublic || $isdelete){ ?>
                    <a href='#' onclick="deletedata('<?php echo $master_product->getId()?>','<?php echo $skip ?>','<?php echo $search ?>')">[Delete]</a>
<?php } ?>
