<?php
    ...
    class master_pay_transferControllerGenerate
    {
        protected $master_pay_transfer;
        var $modulename = "master_pay_transfer";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($master_pay_transfer, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_pay_transfer = $master_pay_transfer;
            $this->dbh = $dbh;            
        ...
        function insertData(){
            ...
            $sql = "INSERT INTO master_pay_transfer ";
            $sql .= " ( ";
	    $sql .= "`id`,";
	    $sql .= "`transfer`,";
	    $sql .= "`img`,";
	    ...
	    $sql .= "`updated_at` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getTransfer(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getImg(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getName_akun(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getRek_akun(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getCreated_at(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getUpdated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_pay_transfer->getUpdated_at(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_pay_transfer SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getId(),$this->dbh)."',";
	    $sql .= "`transfer` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getTransfer(),$this->dbh)."',";
	    $sql .= "`img` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getImg(),$this->dbh)."',";
	    $sql .= "`name_akun` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getName_akun(),$this->dbh)."',";
	    $sql .= "`rek_akun` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getRek_akun(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getCreated_by(),$this->dbh)."',";
	    $sql .= "`created_at` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getCreated_at(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getUpdated_by(),$this->dbh)."',";
	    $sql .= "`updated_at` = '".$this->toolsController->replacecharSave($this->master_pay_transfer->getUpdated_at(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_pay_transfer->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_pay_transfer WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_pay_transfer WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";
        ...
        function checkData($id){
            $sql = "SELECT count(*) FROM master_pay_transfer where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  transfer like '%".$search."%'";
               $where .= "       or  img like '%".$search."%'";
               $where .= "       or  name_akun like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_pay_transfer  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_pay_transfer  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_pay_transfer_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_pay_transfer_ = new master_pay_transfer();
                    $this->loadData($master_pay_transfer_, $row);
                    $master_pay_transfer_List[] = $master_pay_transfer_;
            }
            return $master_pay_transfer_List;            
        }
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_pay_transfer_list = $this->showDataAllLimit();
                ...
                require_once './views/master_pay_transfer/master_pay_transfer_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_pay_transfer/master_pay_transfer_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_pay_transfer_ = $this->showData($id);
                require_once './views/master_pay_transfer/master_pay_transfer_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_pay_transfer_ = $this->showData($id);
                require_once './views/master_pay_transfer/master_pay_transfer_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_pay_transfer_ = $this->showData($id);
                require_once './views/master_pay_transfer/master_pay_transfer_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_pay_transfer_ = $this->showData($id);
                require_once './views/master_pay_transfer/master_pay_transfer_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $name_akun = isset($_POST['name_akun'])?$_POST['name_akun'] : "";
	    $rek_akun = isset($_POST['rek_akun'])?$_POST['rek_akun'] : "";
	    $created_by = isset($_POST['created_by'])?$_POST['created_by'] : "";
	    $created_at = isset($_POST['created_at'])?$_POST['created_at'] : "";
	    $updated_by = isset($_POST['updated_by'])?$_POST['updated_by'] : "";
	    $updated_at = isset($_POST['updated_at'])?$_POST['updated_at'] : "";
	    $this->master_pay_transfer->setId($id);
	    $this->master_pay_transfer->setTransfer($transfer);
	    $this->master_pay_transfer->setImg($img);
	    $this->master_pay_transfer->setName_akun($name_akun);
	    $this->master_pay_transfer->setRek_akun($rek_akun);
	    $this->master_pay_transfer->setCreated_by($created_by);
	    $this->master_pay_transfer->setCreated_at($created_at);
	    $this->master_pay_transfer->setUpdated_by($updated_by);
	    $this->master_pay_transfer->setUpdated_at($updated_at);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_pay_transfer->getId());
            if($check == 0){
        ...
        public function getMaster_pay_transfer() {
            return $this->master_pay_transfer;
        }
        public function setMaster_pay_transfer($master_pay_transfer) {
            $this->master_pay_transfer = $master_pay_transfer;
        }
