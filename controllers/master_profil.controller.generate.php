<?php
    ...
    class master_profilControllerGenerate
    {
        protected $master_profil;
        var $modulename = "master_profil";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $isimport = false;
        var $toolsController;
        function __construct($master_profil, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_profil = $master_profil;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`unitid` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_profil->getNik(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_profil->getUser(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_profil->getAvatar(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_profil->getDepartmentid(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_profil->getUnitid(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_profil SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_profil->getId(),$this->dbh)."',";
	    $sql .= "`nik` = '".$this->toolsController->replacecharSave($this->master_profil->getNik(),$this->dbh)."',";
	    $sql .= "`user` = '".$this->toolsController->replacecharSave($this->master_profil->getUser(),$this->dbh)."',";
	    $sql .= "`avatar` = '".$this->toolsController->replacecharSave($this->master_profil->getAvatar(),$this->dbh)."',";
	    $sql .= "`departmentid` = '".$this->toolsController->replacecharSave($this->master_profil->getDepartmentid(),$this->dbh)."',";
	    $sql .= "`unitid` = '".$this->toolsController->replacecharSave($this->master_profil->getUnitid(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_profil->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_profil WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_profil WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";
        ...
        function checkData($id){
            $sql = "SELECT count(*) FROM master_profil where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  nik like '%".$search."%'";
               $where .= "       or  user like '%".$search."%'";
               $where .= "       or  avatar like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_profil  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_profil  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_profil_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_profil_ = new master_profil();
                    $this->loadData($master_profil_, $row);
                    $master_profil_List[] = $master_profil_;
            }
            return $master_profil_List;            
        }
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_profil_list = $this->showDataAllLimit();
                ...
                require_once './views/master_profil/master_profil_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_profil/master_profil_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                require_once './views/master_profil/master_profil_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_profil_ = $this->showData($id);
                require_once './views/master_profil/master_profil_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_profil_ = $this->showData($id);
                require_once './views/master_profil/master_profil_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
	    ...
	    $this->master_profil->setId($id);
	    $this->master_profil->setNik($nik);
	    $this->master_profil->setUser($user);
	    $this->master_profil->setAvatar($avatar);
	    $this->master_profil->setDepartmentid($departmentid);
	    $this->master_profil->setUnitid($unitid);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_profil->getId());
            if($check == 0){
        ...
        public function getMaster_profil() {
            return $this->master_profil;
        }
        public function setMaster_profil($master_profil) {
            $this->master_profil = $master_profil;
        }
