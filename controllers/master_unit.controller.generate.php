<?php
    ...
    class master_unitControllerGenerate
    {
        protected $master_unit;
        var $modulename = "master_unit";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $isimport = false;
        var $toolsController;
        function __construct($master_unit, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_unit = $master_unit;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`description` ";
            $sql .= ") ";
            $sql .= " VALUES (";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_unit->getUnitid(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_unit->getUnitname(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_unit->getDescription(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_unit SET ";
	    $sql .= "`unitid` = '".$this->toolsController->replacecharSave($this->master_unit->getUnitid(),$this->dbh)."',";
	    $sql .= "`unitname` = '".$this->toolsController->replacecharSave($this->master_unit->getUnitname(),$this->dbh)."',";
	    $sql .= "`description` = '".$this->toolsController->replacecharSave($this->master_unit->getDescription(),$this->dbh)."' ";
            $sql .= "WHERE unitid = '".$this->master_unit->getUnitid()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_unit WHERE unitid = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_unit WHERE unitid = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_unit, $row);
            
            return $this->master_unit;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_unit where unitid = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= " where unitid like '%".$search."%'";
               $where .= "       or  unitname like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_unit  ".$where;
            $sql .= " ORDER BY unitid";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(unitid)  FROM master_unit  ".$where;
            $sql .= " ORDER BY unitid";
            return $sql;
        }
        ...
        function createList($sql){
            $master_unit_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_unit_ = new master_unit();
                    $this->loadData($master_unit_, $row);
                    $master_unit_List[] = $master_unit_;
            }
            return $master_unit_List;            
        }
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_unit_list = $this->showDataAllLimit();
                ...
                require_once './views/master_unit/master_unit_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_unit/master_unit_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_unit_ = $this->showData($id);
                require_once './views/master_unit/master_unit_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_unit_ = $this->showData($id);
                require_once './views/master_unit/master_unit_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_unit_ = $this->showData($id);
                require_once './views/master_unit/master_unit_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_unit_ = $this->showData($id);
                require_once './views/master_unit/master_unit_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $this->master_unit->setUnitid($unitid);
	    $this->master_unit->setUnitname($unitname);
	    $this->master_unit->setDescription($description);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_unit->getUnitid());
            if($check == 0){
        ...
        public function getMaster_unit() {
            return $this->master_unit;
        }
        public function setMaster_unit($master_unit) {
            $this->master_unit = $master_unit;
        }
