<?php
    ...
    class master_departmentControllerGenerate
    {
        protected $master_department;
        var $modulename = "master_department";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        function __construct($master_department, $dbh) {
            ...
            $this->master_department = $master_department;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`description` ";
            $sql .= ") ";
            $sql .= " VALUES (";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_department->getDepartmentid(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_department->getDescription(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_department SET ";
	    $sql .= "`departmentid` = '".$this->toolsController->replacecharSave($this->master_department->getDepartmentid(),$this->dbh)."',";
	    $sql .= "`description` = '".$this->toolsController->replacecharSave($this->master_department->getDescription(),$this->dbh)."' ";
            $sql .= "WHERE departmentid = '".$this->master_department->getDepartmentid()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_department WHERE departmentid = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_department WHERE departmentid = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_department, $row);
            
            return $this->master_department;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_department where departmentid = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= " where departmentid like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_department  ".$where;
            $sql .= " ORDER BY departmentid";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(departmentid)  FROM master_department  ".$where;
            $sql .= " ORDER BY departmentid";
            return $sql;
        }
        ...
        function createList($sql){
            $master_department_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_department_ = new master_department();
                    $this->loadData($master_department_, $row);
                    $master_department_List[] = $master_department_;
            }
            return $master_department_List;            
        }
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_department_list = $this->showDataAllLimit();
                ...
                require_once './views/master_department/master_department_list.php';
            }else{
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_department_ = $this->showData($id);
                require_once './views/master_department/master_department_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_department_ = $this->showData($id);
                require_once './views/master_department/master_department_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_department_ = $this->showData($id);
                require_once './views/master_department/master_department_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_department_ = $this->showData($id);
                require_once './views/master_department/master_department_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
        ...
        public function saveData(){
            $check = $this->checkData($this->master_department->getDepartmentid());
            if($check == 0){
        ...
        public function setMaster_department($master_department) {
            $this->master_department = $master_department;
        }
