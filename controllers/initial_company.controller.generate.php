<?php
    ...
    class initial_companyControllerGenerate
    {
        protected $initial_company;
        var $modulename = "initial_company";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($initial_company, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->initial_company = $initial_company;
            $this->dbh = $dbh;            
        ...
        function insertData(){
            ...
            $sql = "INSERT INTO initial_company ";
            $sql .= " ( ";
	    $sql .= "`id`,";
	    $sql .= "`company_name`,";
	    $sql .= "`username`,";
	    ...
	    $sql .= "`bgfile` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->initial_company->getCompany_name(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->initial_company->getUsername(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->initial_company->getDatabase_name(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->initial_company->getLogo(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->initial_company->getBgfile(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE initial_company SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->initial_company->getId(),$this->dbh)."',";
	    $sql .= "`company_name` = '".$this->toolsController->replacecharSave($this->initial_company->getCompany_name(),$this->dbh)."',";
	    $sql .= "`username` = '".$this->toolsController->replacecharSave($this->initial_company->getUsername(),$this->dbh)."',";
	    $sql .= "`database_name` = '".$this->toolsController->replacecharSave($this->initial_company->getDatabase_name(),$this->dbh)."',";
	    $sql .= "`logo` = '".$this->toolsController->replacecharSave($this->initial_company->getLogo(),$this->dbh)."',";
	    $sql .= "`bgfile` = '".$this->toolsController->replacecharSave($this->initial_company->getBgfile(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->initial_company->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM initial_company WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM initial_company WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->initial_company, $row);
            
            return $this->initial_company;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM initial_company where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  company_name like '%".$search."%'";
               $where .= "       or  username like '%".$search."%'";
               $where .= "       or  database_name like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM initial_company  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM initial_company  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $initial_company_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $initial_company_ = new initial_company();
                    $this->loadData($initial_company_, $row);
                    $initial_company_List[] = $initial_company_;
            }
            return $initial_company_List;            
        }
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $initial_company_list = $this->showDataAllLimit();
                ...
                require_once './views/initial_company/initial_company_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/initial_company/initial_company_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $initial_company_ = $this->showData($id);
                require_once './views/initial_company/initial_company_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $initial_company_ = $this->showData($id);
                require_once './views/initial_company/initial_company_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $initial_company_ = $this->showData($id);
                require_once './views/initial_company/initial_company_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $initial_company_ = $this->showData($id);
                require_once './views/initial_company/initial_company_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $bgfile = isset($_POST['bgfile'])?$_POST['bgfile'] : "";
	    $this->initial_company->setId($id);
	    $this->initial_company->setCompany_name($company_name);
	    $this->initial_company->setUsername($username);
	    $this->initial_company->setDatabase_name($database_name);
	    $this->initial_company->setLogo($logo);
	    $this->initial_company->setBgfile($bgfile);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->initial_company->getId());
            if($check == 0){
        ...
        public function getInitial_company() {
            return $this->initial_company;
        }
        public function setInitial_company($initial_company) {
            $this->initial_company = $initial_company;
        }
